import { e, handleError, rotate90DegreesClockwise } from "../";
...
export default class Matrix {
  ...
  constructor(
    ...
    if (rows === undefined && columns === undefined && data === undefined) {
      // Case 1: No arguments provided, create a 1x1 matrix with zero
      this._initializeMatrix(1, 1, [[0]]);
    } else if (rows !== undefined && typeof rows === "number") {
      if (Array.isArray(columns) && data === undefined) {
        // Case 2: Rows and data provided, create a square matrix with the provided data
        this._initializeMatrix(rows, rows, columns);
      } else if (columns !== undefined && typeof columns === "number") {
        if (Array.isArray(data)) {
          // Case 3: Rows, columns, and data provided, create a matrix with the provided data
          this._initializeMatrix(rows, columns, data);
        } else {
          // Case 4: Rows and columns provided, create a matrix with zeroes
          this._initializeMatrix(rows, columns, []);
        }
      } else {
        // Case 5: Only rows provided, create a square matrix with zeroes
      ...
      // Case 6: Rows provided as an array, create a matrix with the provided data
      this._initializeMatrix(rows.length, rows[0].length, rows);
    } else {
      // Case 7: Invalid arguments provided
      handleError(e.INVALID_ARGUMENTS);
    }
  }
   ...
   */
  setData(data: number[][], override?: boolean) {
    if (!override) {
      handleError(
  ...
  add(matrix: Matrix): Matrix {
    ...
    const result: number[][] = Array.from({ length: this._rows }, (_, i) =>
      Array.from(
        { length: this._columns },
        (_, j) => this.at(i, j) + matrix.at(i, j)
      )
    );
  ...
  multiply(matrixOrScalar: Matrix | number): Matrix {
    if (typeof matrixOrScalar === "number") {
      ...
      const result: number[][] = this._data.map((row) =>
        row.map((value) => value * matrixOrScalar)
      );
      ...
      const result: number[][] = Array.from({ length: this._rows }, (_, i) =>
        Array.from({ length: matrixOrScalar.columns }, (_, j) =>
          this._data[i].reduce(
  ...
  isIdentity(): boolean {
    ...
    return this._data.every((row, i) =>
      row.every((value, j) => (i === j ? value === 1 : value === 0))
    );
  }
  ...
  setShape(shape: number[]) {
    handleError(e.MATRIX_SHAPE_MISMATCH, shape.length !== 2);
   ...
   */
  setAsIdentity(): void {
    handleError(e.NON_SQUARE_MATRIX, !this.isSquare());
   ...
   */
  transpose(): Matrix {
    const result: number[][] = Array.from({ length: this._columns }, (_, i) =>
      this._data.reduce((column, row) => {
        column.push(row[i]);
        return column;
      }, [])
    );
  ...
  determinant(): number {
    ...
    if (this._rows === 1) {
      ...
      // Case 3: nxn matrix
      let determinant: number = this._data[0].reduce(
  ...
  getCofactor(row: number, column: number): number {
    const minor: Matrix = this.getMinor(row, column);
  ...
  getMinor(row: number, column: number): Matrix {
    let result: number[][] = this._data
      .filter((_, i) => i !== row)
      .map((v) => v.filter((_, j) => j !== column));
  ...
  trace(): number {
    ...
    let trace: number = Array.from({ length: this._rows }, (_, i) =>
      this.at(i, i)
  ...
  inverse(): Matrix {
    ...
    handleError(e.ZERO_MATRIX_DETERMINANT("inverse"), determinant === 0);

    const result: number[][] = this._data.map((v, i) =>
      v.map((_, j) => this.getCofactor(i, j) / determinant)
    );
   ...
   */
  rowSpace(): Matrix {
    const result: number[][] = this._data.map((v) => v.slice());
   ...
   */
  colSpace(): Matrix {
    const result: number[][] = this._data[0].map((_, i) =>
      this._data.map((v) => v[i])
    );
  ...
  rotate(angle: number): Matrix {
    let angleInDegrees: number =
      Math.abs(angle) <= Math.PI ? angle * (180 / Math.PI) : angle;

    handleError(e.INVALID_ROTATE_ANGLE, angleInDegrees % 90 !== 0);
  ...
  equals(matrix: Matrix): boolean {
    ...
    return this._data.every((v, i) =>
      v.every((_, j) => this.at(i, j) === matrix.at(i, j))
    );
  }
   ...
   * @private
   */
  private _isOutOfBounds(row: number, column: number): void {
    handleError(
   ...
   * @private
   */
  private _initializeMatrix(rows: number, columns: number, data: number[][]) {
    this._rows = rows;
